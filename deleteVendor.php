...
$re=mysqli_query($con,"select * from fuel where fldvendor_id='$Vendorid'");
$re_arr=mysqli_fetch_array($re);
$re_no=mysqli_num_rows($re);
if($re_no)
{
	unset($_SESSION['id']);
	 $vend_info=mysqli_query($con,"select * from vendor where fldvendor_id='$Vendorid'");
	 $vend_row=mysqli_fetch_array($vend_info);
	  $im=$vend_row['fld_logo'];
	  $em=$vend_row['fld_email'];
	  $food_img=$re_arr['fldimage'];
	 unlink("image/pump/$em/foodimages/$food_img");
	// rmdir("image/pump/$em/foodimages");
	 unlink("image/pump/$em/$im");
	 //rmdir("image/pump/$em");
	 mysqli_query($con,"delete from fuel where fldvendor_id='$Vendorid'");
	 mysqli_query($con,"delete from vendor where fldvendor_id='$Vendorid'");
