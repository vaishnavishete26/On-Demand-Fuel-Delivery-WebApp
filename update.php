...
{
if(!empty($_GET['fuel_id']))
{
	$fuel_id=$_GET['fuel_id'];
	$query=mysqli_query($con,"select * from fuel   where fuel_id='$fuel_id'");
if(mysqli_num_rows($query))
{   
	 $row=mysqli_fetch_array($query);
     $rfuelname=$row['fuelname'];
     $rcost=$row['cost'];
   ...
   if(!empty($_SESSION['id']))	
   {
    ...
    if(!empty($chk)) 
	{
		if(empty($img_name))
	       ...
	       {
		          $paymentmode=implode(",",$chk);
	              if(mysqli_query($con,"update  fuel  set fuelname='$food_name',cost='$cost',quantity='$quantity',paymentmode='$paymentmode' where fuel_id='$fuel_id'"))
	   
	                {
						header("location:update_fuel.php?fuel_id=$fuel_id");
		              //echo "update with old pic";
		              //move_uploaded_file($_FILES['food_pic']['tmp_name'],"../image/pump/$em/foodimages/".$_FILES['food_pic']['name']);
	                 }
	              else{
		               echo "failed";
	                  }
	     ...
	     else
		 {
			     ...
			     echo $paymentmode."<br>";
			     echo $img_name."<br>";
	             if(mysqli_query($con,"update  fuel  set fuelname='$food_name',cost='$cost',quantity='$quantity',paymentmode='$paymentmode', fldimage='$img_name' where fuel_id='$fuel_id'"))
	
	                {
		             echo "update with new pic";
		             move_uploaded_file($_FILES['food_pic']['tmp_name'],"image/pump/$em/foodimages/".$_FILES['food_pic']['name']);
	                 unlink("image/pump/$em/foodimages/$rfldimageold");
					 header("location:update_fuel.php?fuel_id=$fuel_id");
					}
				 else
				 {
...
<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
    ...
    <a class="navbar-brand" href="../index.php"><span style="color:green;font-family: 'Permanent Marker', cursive;">On Demand Fuel Delivery</span></a>
            ...
            <div class="tab-pane fade show active" id="viewitem" role="tabpanel" aria-labelledby="home-tab">
                  <!--add Product-->
                        <form action="" method="post" enctype="multipart/form-data">
                                     ...
                                     <label for="food_name">Fuel Name:</label>
                                            <input type="text" class="form-control" id="food_name" value="<?php if(isset($rfuelname)) { echo $rfuelname;}?>" placeholder="Enter Food Name" name="food_name" required>
                                     </div>
	                                 ...
	                                 <div class="form-group"><!--quantity-->
                                            ...
                                            <input type="text" class="form-control" id="quantity" value="<?php if(isset($rquantity)) { echo $rquantity;}?>" placeholder="Enter quantity" name="quantity" required>
                                    </div>
									...
									<?php
			                          ...
			                          $pay=explode(",",$rpaymentmode);
			                           ...
			                           ?>
			                             ...
			                             <input type="checkbox" <?php if(in_array("Online Payment",$pay)) { echo "checked"; } ?> name="chk[]" value="Card Payment"/>Card Payment
								         <br>
	                                ...
	                                <div class="form-group">
                                         ...
                                         <input type="file" accept="image/*" name="food_pic"/>Fuel Snaps 
                                    </div>
