...
{
 $id=$_SESSION['id'];
 $vq=mysqli_query($con,"select * from vendor where fld_email='$id'");
...
{
$query=mysqli_query($con,"select * from vendor   where fld_email='$id'");
if(mysqli_num_rows($query))
{   if(!file_exists("image/pump/".$id."/foodimages"))
	{
		$dir=mkdir("image/pump/".$id."/foodimages");
	}
...
{   if(isset($_SESSION['id']))
	...
	if(mysqli_query($con,"insert into fuel(fldvendor_id,fuelname,cost,quantity,paymentmode,fldimage) values
	
	('$v_id','$food_name','$cost','$quantity','$paymentmode','$img_name')"))
	{
		move_uploaded_file($_FILES['food_pic']['tmp_name'],"image/pump/$id/foodimages/".$_FILES['food_pic']['name']);
	}
	else{
		echo "failed";
	}
...
if(isset($upd_account))
				{
					...
					//echo $emm;
					//echo $add;
					if(mysqlI_query($con,"update vendor set fld_name='$fn',fld_email='$emm',fld_address='$add',fld_mob='$mob',fld_password='$pwsd' where fld_email='$id'"))
				   {
			  ...
			  if(isset($upd_logo))
			  {
				  if(isset($_SESSION['id']))
				  {
				  $log_img=mysqli_query($con,"select * from vendor where fld_email='$id'");
                  $log_img_row=mysqli_fetch_array($log_img);
				  $old_logo=$log_img_row['fld_logo'];
				  $new_img_name=$_FILES['logo_pic']['name'];
				  
				  if(mysqli_query($con,"update vendor set fld_logo='$new_img_name' where fld_email='$id'"))
				  {
					  unlink("image/pump/$id/$old_logo");
					  move_uploaded_file($_FILES['logo_pic']['tmp_name'],"image/pump/$id/".$_FILES['logo_pic']['name']);
...
<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
    ...
    <a class="navbar-brand" href="home.php"><span style="color:green;font-family: 'Permanent Marker', cursive;">On Deamnd Fuel Delivery</span></a>
    <?php
	if(!empty($id))
	{
	?>
	   ...
	   <div class="tab-content" id="myTabContent">
            ...
            <div class="tab-pane fade show active" id="viewitem" role="tabpanel" aria-labelledby="home-tab">
			 ...
			 <th>Update item Details </th>
			   <?php
					  if($query=mysqli_query($con,"select vendor.fldvendor_id,vendor.fld_email,fuel.fuel_id,fuel.fuelname,fuel.cost,fuel.paymentmode,fuel.quantity,fuel.fldimage,vendor.fld_name from vendor inner join fuel on vendor.fldvendor_id=fuel.fldvendor_id where vendor.fld_email='$id'"))
					  {
						  if(mysqli_num_rows($query))
						  {
				...
				<td><img src="<?php echo 'image/pump/'.$id.'/foodimages/'.$row['fldimage'];?>" height="100px" width="150px"></td>
				<td style="width:150px;"><?php  echo $row['fuelname']."<br>";?></td>
				<td align="center" style="width:150px;"><?php  echo $row['cost']."<br>";?></td>
				<td  align="center" style="width:150px;"><?php  echo $row['quantity']."<br>";?></td>
				<td align="center" style="width:150px;"><?php  echo $row['paymentmode']."<br>";?></td>
				
				<td align="center" style="width:150px;">
				
				<a href="vendor_delete_fuel.php?fuel_id=<?php echo $row['fuel_id'];?>"><button type="button" class="btn btn-warning">Delete </button></a>
				
				</td>
				<td align="center" style="width:150px;">
				
				<a href="update.php?fuel_id=<?php echo $row['fuel_id'];?>"><button type="button" class="btn btn-danger">Update </button></a>
...
<!--tab 1 ends-->	   
            ...
            <div class="tab-pane fade" id="manageaccount" role="tabpanel" aria-labelledby="profile-tab">
			         <!--add Product-->
                        <form action="" method="post" enctype="multipart/form-data">
                                     ...
                                     <label for="food_name">Fuel Type:</label>
                                            <input type="text" class="form-control" id="food_name" value="<?php if(isset($food_name)) { echo $food_name;}?>" placeholder="Enter fuel type" name="food_name" required>
                                     </div>
	                                 ...
	                                 <div class="form-group"><!--quantity-->
                                            ...
                                            <input type="text" class="form-control" id="quantity" value="<?php if(isset($quantity)) { echo $quantity;}?>" placeholder="Enter quantity" name="quantity" required>
                                    </div>
							        
							        <div class="form-group"><!--payment_mode-->
                                         <input type="checkbox" name="chk[]" value="COD"/>Cash On Delivery
			                             <input type="checkbox" name="chk[]" value="Online Payment"/>Card Payment
								         <br>
								        <span style="color:red;"><?php if(isset($paymessage)){ echo $paymessage;}?></span>
			      			        </div>
							   
	                                <div class="form-group">
                                         <input type="file" accept="image/*" name="food_pic" required/>Product Snaps 
                                    </div>
			...
			<!--tab 2 ends-->
			 ...
			 <div class="tab-pane fade" id="accountsettings" role="tabpanel" aria-labelledby="accountsettings-tab">
				...
				<?php
			    $upd_info=mysqli_query($con,"select * from vendor where fld_email='$id'");
			...
			<div class="tab-pane fade" id="logo" role="tabpanel" aria-labelledby="logo-tab">
			 ...
			 <div class="tab-pane fade " id="status" role="tabpanel" aria-labelledby="status-tab">
				...
				<th>Update Status</th>
				<?php
				$orderquery=mysqli_query($con,"select * from myorder where fldvendor_id='$vrid'");
				if(mysqli_num_rows($orderquery))
				{
					while($orderrow=mysqli_fetch_array($orderquery))
					{
						...
						<td><?php echo $orderrow['fld_fuel_id']; ?></td>
						<?php
			 ...
			 <table class="table">
				...
				<th>Address</th>
				<?php
				//$custquery=mysqli_query($con,"select * from tblcustomer where fld_email='$orderrow['fld_email_id']'");
					$custquery=mysqli_query($con,"select * from customer where fld_email IN (select fld_email_id from myorder where fldvendor_id= $vrid)");
				

				if(mysqli_num_rows($custquery))
				{
					while($custrow=mysqli_fetch_array($custquery))
					{
						
						?>
						<tr>
						<td><?php echo $custrow['fld_name']; ?></td>
						<td><?php echo $custrow['fld_email']; ?></td>
						<td><?php echo $custrow['fld_mobile']; ?></td>
